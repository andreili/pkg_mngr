--
-- Файл сгенерирован с помощью SQLiteStudio v3.0.7 в Ср сен 14 16:41:02 2016
--
...
INSERT INTO config_aliases (id, alias, on_value, off_value) VALUES (6, 'FLAGOFF', NULL, '${OPT}');
INSERT INTO config_aliases (id, alias, on_value, off_value) VALUES (7, 'FLAGON', '${OPT}', NULL);
...
INSERT INTO pkg_opts (id, pkg_id, opt_id, "default", opt_alias_id) VALUES (36, 6, 18, 0, NULL);
...
INSERT INTO pkg_opts (id, pkg_id, opt_id, "default", opt_alias_id) VALUES (56, 10, 36, 0, NULL);
...
INSERT INTO config_cmds (id, pkg_id, cmd, dir, dep_by_opt_id) VALUES (10, 11, 'ac_cv_have_x=''have_x=yes ac_x_includes= ac_x_libraries='' ${SRC_DIR}/${PS}-${PV}/configure --host=$(gcc -dumpmachine) --build=$(gcc -dumpmachine) --target=$(gcc -dumpmachine) --prefix=/usr --enable-languages=c,c++ --disable-multilib --disable-bootstrap --with-system-zlib --bindir=/usr/$(gcc -dumpmachine)/gcc-bin/${PV} --includedir=/usr/include --datadir=/usr/share/gcc-data/$(gcc -dumpmachine)/${PV} --mandir=/usr/share/gcc-data/$(gcc -dumpmachine)/${PV}/man --infodir=/usr/share/gcc-data/$(gcc -dumpmachine)/${PV}/info --with-gxx-include-dir=/usr/lib/gcc/$(gcc -dumpmachine)/${PV}/include/g++-v${PV} --with-python-dir=/usr/share/gcc-data/$(gcc -dumpmachine)/${PV}/python --enable-languages=c${OPT cxx FLAGON ,c++}${OPT go FLAGON ,go}${OPT objc FLAGON ,objc}${OPT objc++ FLAGON ,obj-c++}${OPT fortran FLAGON ,fortran} --enable-obsolete --enable-secureplt --disable-werror ${OPT nls ENDIS nls} ${OPT nls WITHIO included-gettext} --disable-libunwind-exceptions ${OPT cxx ENDIS libstdcxx-time} ${OPT cxx ENDIS build-with-cxx} ${OPT cxx ENDIS build-poststage1-with-cxx} --enable-shared --enable-threads=posix --disable-libgcj ${OPT openmp ENDIS libgomp} ${OPT libssp ENDIS libssp} ${OPT cilk ENDIS libcilkrts} ${OPT fortran FLAGOFF --disable-libquadmath} --enable-lto ${OPT graphite ENDIS isl} ${OPT graphite FLAGON --disable-isl-version-check} ${OPT sanitize ENDIS libsanitizer} ${OPT pie ENDIS default-pie}', '${BUILD_DIR}', NULL);
